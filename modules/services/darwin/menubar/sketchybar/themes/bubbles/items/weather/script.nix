...
in pkgs.writeShellScript "weather-script.sh" ''
  ...
  render_item() {
    if [ "$TEMP" = "" ]; then
      ...
      args=(--set "$NAME" icon="$ICON" icon.font="${cfg.font.family}:Bold:${toString (cfg.font.size - 2)}" label="''${TEMP}°" label.drawing=on)
    fi
